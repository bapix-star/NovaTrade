...
export class OrderContractClient {
  ...
  async createOrder(params: {
    ...
    return stellar.buildAndSignTx({
      ...
      method: 'create_order',
      args: [
        StellarSdk.Address.fromString(params.publicKey).toScVal(),
        StellarSdk.Address.fromString(params.supplier).toScVal(),
        StellarSdk.Address.fromString(params.shipper).toScVal(),
        StellarSdk.Address.fromString(params.inspector).toScVal(),
  ...
  async inspectOrder(publicKey: string, orderId: number, passed: boolean): Promise<{ hash: string }> {
    return stellar.buildAndSignTx({
      ...
      args: [
        StellarSdk.Address.fromString(publicKey).toScVal(),
        StellarSdk.nativeToScVal(orderId, { type: 'u64' }),
        StellarSdk.nativeToScVal(passed),
      ],
    });
  }
